"""OpenAI Responses API implementation of `PersonaLLMProvider`.
...
Even though the request asks the provider for structured JSON output,
that alone is not trusted: the raw output text is parsed with `json.loads`
and validated against `PersonaGenerationResult` locally, exactly as if the
provider had returned unstructured text.
"""
...
from app.llm.prompts import build_system_instructions, build_user_prompt
...
class OpenAIPersonaProvider:
    """Persona generation via the OpenAI Responses API."""
    ...
    def generate_personas(
        ...
        *,
        ...
        system_instructions = build_system_instructions(persona_count)
        user_prompt = build_user_prompt(context, focus)

        logger.info(
            "Requesting persona generation (model=%s, persona_count=%d, evidence_count=%d)",
            self.model_name,
            persona_count,
        ...
        try:
            ...
            raise LLMInvalidOutputError("The AI provider returned malformed JSON.") from exc

        try:
            return PersonaGenerationResult.model_validate(
